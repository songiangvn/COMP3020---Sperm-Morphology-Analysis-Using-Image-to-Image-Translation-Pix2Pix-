"""
Quick inference script for cGAN to check results
"""
import os
import torch
...
def load_cgan_model(checkpoint_path, device):
    """Load trained cGAN generator"""
    model = Generator(in_channels=3, out_channels=3).to(device)
    checkpoint = torch.load(checkpoint_path, map_location=device)
    model.load_state_dict(checkpoint['generator_state_dict'])
    model.eval()
    print(f"Loaded cGAN from epoch {checkpoint['epoch']+1}")
    print(f"Val G Loss: {checkpoint.get('val_g_loss', 'N/A'):.4f}")
    print(f"Val D Loss: {checkpoint.get('val_d_loss', 'N/A'):.4f}")
    return model
...
def visualize_results(results, output_dir='./inference_cgan'):
    ...
    for i, result in enumerate(results):
        ...
        axes[i, 1].set_title('cGAN Generated')
        axes[i, 1].axis('off')
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'cgan_samples.png')
...
if __name__ == '__main__':
    ...
    print("="*60)
    print("cGAN Inference - Quick Check")
    print("="*60)
    
    # Load model
    model = load_cgan_model(checkpoint_path, device)
